...
def f_setPath(folder):
    ...
    path_student = "//ssimarfile01/CC/CF_WERK/TMP/Studenti/2018_Leon/DATA/05_ZPRMA1"
...
def f_Pretvori_v_dataframe(datoteka_vhodna):
    ...
    for index, row in df.iterrows():
        ...
        if n == n0: #dokler si znotraj enega dne (n šteje dneve)
            ...
            elif (("BOLNI" in str(urnik).upper()) or ("REFUNDACIJA" in str(urnik).upper())) and ("->" not in str(urnik)):
                dfA.loc[0] = [sDatum, MS, priimek, "0:00", "0:00", sErr, "0", sTipUrnika, sPrisotnost, sPlan, sPrisotnost, "0:00", "0:00" ]
                df2 = df2.append(dfA, ignore_index=True)   
            elif (("DOPUST" in str(urnik).upper()) or ("CELODNEVNO" in str(urnik).upper())) and ("->" not in str(urnik)):
...
def main():
    try:
        ...
        #arguments from cmd line
            ...
            datoteka_vhodna = './test_data.txt'
            datoteka_izhodna = './results/data_ZPRMA1_dogodki.csv'

        sLogFile = 'ED_log.txt' #trenutno se ne uporabljam
        ...
        f_Uredi_SpicaTXT(datoteka_vhodna, datoteka_tmp1) #rezultat predelave shranim v tmp1
        
        print ('Mimo uredi')
        
        df2 = f_Pretvori_v_dataframe(datoteka_tmp1)
        
        print ('Mimo pretvori v dataframe')
        
        df2['H_start'] = df2['prihod'].apply(fUraDec) #zacetek dela v decimalni obliki
        df2['H_end'] = df2['odhod'].apply(fUraDec) #konec dela v decimalni obliki
        df2['H_1'] = df2.apply(lambda x: fInterval(x.H_start, x.H_end)[0], axis=1) #prva izmena
        df2['H_2'] = df2.apply(lambda x: fInterval(x.H_start, x.H_end)[1], axis=1) #druga izmena
        df2['H_3'] = df2.apply(lambda x: fInterval(x.H_start, x.H_end)[2], axis=1) #tretja izmena
        df2['H_bolniska'] = df2['bolniska'].apply(fUraDec)
        df2['H_dopust'] = df2['dopust'].apply(fUraDec)
        df2['H_skupaj'] = df2['H_1'] + df2['H_2'] + df2['H_3'] + df2['H_bolniska'] + df2['H_dopust']

        #ta je samo za kontrolo
        ...
        df3.reset_index(level=df3.index.names, inplace=True) #one can convert a given whole multiindex into columns 
        ...
        #df3.to_csv(datoteka_izhodna, mode='w', sep=';', header=True, index=True, encoding='cp1250', decimal=',' )
        df_u = pd.read_excel(datoteka_uporabniki, sheet_name='DATA')
        ...
        df5.reset_index(level=df5.index.names, inplace=True) 
        df5.to_csv(datoteka_izhodna, mode='w', sep=';', header=True, index=False, encoding='cp1250', decimal=',' )   
